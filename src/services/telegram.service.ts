	...
	private async setupWebhook() {
		...
		if (webhookUrl) {
			await this.bot.telegram.setWebhook(webhookUrl, {
				secret_token: secret, // Telegram will send this back in the header
				// drop_pending_updates: true, // uncomment if you want a clean slate
			});
...
Let's start tracking your work hours! 🚀
      `;
		...
		this.bot.command('status', async (ctx) => {
			try {
				const status = await this.timeTrackingService.getTodayStatus(ctx.from.id.toString());
